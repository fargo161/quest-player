...
function observe(page) {
  ...
  page.on('console', message => {
    if (message.type() === 'error') {
      ...
      if (location.includes('missing-intro-video.mp4') || location.includes('content-media.example.test')) evidence.expectedMediaFailures.push(message.text());
      else unexpected.push(`console: ${message.text()} ${location}`);
    }
  });
  page.on('requestfailed', request => {
    if (request.url().includes('missing-intro-video.mp4') || request.url().includes('content-media.example.test')) evidence.expectedMediaFailures.push(request.failure()?.errorText || 'media failed');
...
try {
  ...
  await staffContext.route('https://content-media.example.test/**', route => route.abort('failed'));
  ...
  await staff.getByRole('link', { name: 'Edit player content' }).click();
  await staff.getByRole('heading', { name: 'Player content', exact: true }).waitFor();
  for (const state of ['NONE','ACTIVE','COMPLETED','ELIGIBLE','SELECTED','FULFILLED']) {
    await staff.goto(`${config.baseUrl}/admin/content/as-above-so-below/${state}`);
    const expectedVersion = Number(await staff.locator('input[name="version"]').inputValue()) + 1;
    await staff.getByLabel('Title', { exact: true }).fill(`Demo ${state} content`);
    await staff.getByLabel('Plain text', { exact: true }).fill(`Instructions for ${state}. This is synthetic browser test content.`);
    await staff.getByLabel('Image URL (HTTPS)', { exact: true }).fill(state === 'ACTIVE' ? 'https://content-media.example.test/missing-image.png' : '');
    await staff.getByLabel('Image description', { exact: true }).fill(state === 'ACTIVE' ? 'Synthetic quest image description' : '');
    await staff.getByLabel('Video URL (HTTPS)', { exact: true }).fill(state === 'ACTIVE' ? 'https://content-media.example.test/missing-intro-video.mp4' : '');
    await staff.getByLabel('Reason for this change', { exact: true }).fill('Set synthetic state content for browser verification');
    await Promise.all([staff.waitForResponse(response => response.request().method() === 'POST' && response.url().includes('/admin/content/')), staff.getByRole('button', { name: 'Save and publish content' }).click()]);
    await staff.locator(`input[name="version"][value="${expectedVersion}"]`).waitFor({ state: 'attached' });
    await staff.getByText(`Demo ${state} content`, { exact: true }).first().waitFor();
  }
  await capture(staff, '06-mobile-content-editor');
  evidence.checks.push('Staff editor publishes six distinct participation/reward content slots and displays immutable version history');
  const playerContext = await browser.newContext({ viewport: evidence.viewport, isMobile: true, deviceScaleFactor: 1, hasTouch: true });
  await playerContext.route('https://content-media.example.test/**', route => route.abort('failed'));
  ...
  await player.getByRole('heading', { name: 'Demo NONE content', exact: true }).waitFor();
  assert.equal(await player.getByRole('heading', { name: 'Demo ACTIVE content', exact: true }).count(), 0);
  ...
  await player.getByRole('heading', { name: 'Demo ACTIVE content', exact: true }).waitFor();
  assert.equal(await player.getByRole('img', { name: 'Synthetic quest image description' }).getAttribute('alt'), 'Synthetic quest image description');
  ...
  await staff.goto(`${config.baseUrl}/admin`);
  ...
  await player.reload();
  await player.getByRole('heading', { name: 'Demo COMPLETED content', exact: true }).waitFor();
  await player.getByRole('heading', { name: 'Demo ELIGIBLE content', exact: true }).waitFor();
  assert.equal(await player.getByRole('heading', { name: 'Demo ACTIVE content', exact: true }).count(), 0);
  ...
  await player.reload();
  await player.getByRole('heading', { name: 'Demo SELECTED content', exact: true }).waitFor();
  assert.equal(await player.getByRole('heading', { name: 'Demo ELIGIBLE content', exact: true }).count(), 0);
  ...
  await player.getByRole('heading', { name: 'Demo FULFILLED content', exact: true }).waitFor();
  await player.getByRole('heading', { name: 'Demo COMPLETED content', exact: true }).waitFor();
  assert.equal(await player.getByRole('heading', { name: 'Demo SELECTED content', exact: true }).count(), 0);
  await capture(player, '05-mobile-fulfilled-profile');
  evidence.checks.push('Database state selects participation and reward content independently; failed HTTPS image/video retain text and allow progress');
